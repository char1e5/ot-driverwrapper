...
exports.getBrowserWithUA = function (browser, user_agent) {
    ...
    if (browser === 'chrome') {
        var opts = new chrome.Options().addArguments(['user-agent="' + user_agent + '"']);
        service = new chrome.ServiceBuilder().loggingTo('C:\\chromedriver.log').enableVerboseLogging().build();
        driver = chrome.createDriver(opts, service);
        return new DriverWrapper(driver);
    } else if (browser === 'phantomjs') {
        var opts = webdriver.Capabilities.phantomjs().set('phantomjs.page.settings.userAgent', user_agent);
        driver = new webdriver.Builder().withCapabilities(opts). build();
        return new DriverWrapper(driver);
    } else {
        console.log('Only support Chrome and PhantomJS');
        return null;
    }
}
